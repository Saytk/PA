...
public:
                        ...
                        int num_neurons_per_hidden_layer, double learning_rate, string activation_func_name)
    {
        ...
        // The softmax function should be applied to a vector of values, not to each value individually.
        // Therefore, it does not fit well in this map. It will need to be handled separately.
        // If you don't plan to use softmax in hidden layers, this should not be a problem.
    ...
    void train(Eigen::VectorXd input, Eigen::VectorXd target)
    {
        ...
        for (int i = 0; i < layers.size(); i++)
        {
            output = layers[i].weights.transpose() * output + layers[i].bias;
            output = layers[i].activation(output);
            outputs.push_back(output);
        }
        ...
        for (int i = layers.size() - 1; i >= 0; i--)
        {
            ...
            // Note: For a matrix A and vectors x, y, the outer product y*A*x is a matrix. 
            // However, for a matrix A and vectors x, y, the expression y.transpose()*A*x is a scalar.
            // So, to keep dimensions consistent, we need to use outer product here.
